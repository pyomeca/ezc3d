...
TEST(ForcePlatForm, Type2) {
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].x(), 1017.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].y(), 464.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].z(), 0.0);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].x(), 1017.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].y(), 0.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].z(), 0.0);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].x(), 509.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].y(), 0.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].z(), 0.0);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].x(), 509.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].y(), 464.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].z(), 0.0);

  const auto &meanCorners(pf.forcePlatform(1).meanCorners());
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.x(), 763.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.y(), 232.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.z(), 0.0);

  const auto &origin(pf.forcePlatform(1).origin());
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.x(), 1.016);
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.y(), 0.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.z(), -36.321999);
}
...
TEST(ForcePlatForm, Type3) {
  ...
  const auto &corners(pf.forcePlatform(1).corners());
  EXPECT_EQ(corners.size(), 4);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].x(), 1785.6);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].y(), 587.117);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].z(), -5.1672401);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].x(), 1787.55);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].y(), 0.299824);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].z(), -5.80374);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].x(), 902.06897);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].y(), -2.4895);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].z(), -6.3048501);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].x(), 900.30902);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].y(), 582.18701);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].z(), -4.9547);

  const auto &meanCorners(pf.forcePlatform(1).meanCorners());
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.x(), 1343.882);
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.y(), 291.77859);
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.z(), -5.5576324);

  const auto &origin(pf.forcePlatform(1).origin());
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.x(), 210.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.y(), 350.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.z(), -53.0);
}
...
TEST(ForcePlatForm, Type4) {
  ...
  const auto &corners(pf.forcePlatform(1).corners());
  EXPECT_EQ(corners.size(), 4);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].x(), -293.892);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].y(), 1403.719);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[0].z(), -3.9355);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].x(), 265.108);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].y(), 1402.601);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[1].z(), -3.3765);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].x(), 261.552);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].y(), -375.399);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[2].z(), 3.7355);

  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].x(), -297.448);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].y(), -374.281);
  EXPECT_DOUBLE_AS_FLOAT_EQ(corners[3].z(), 3.1765);

  const auto &meanCorners(pf.forcePlatform(1).meanCorners());
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.x(), -16.17);
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.y(), 514.16);
  EXPECT_DOUBLE_AS_FLOAT_EQ(meanCorners.z(), -0.0999999);

  const auto &origin(pf.forcePlatform(1).origin());
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.x(), -279.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.y(), 889.0);
  EXPECT_DOUBLE_AS_FLOAT_EQ(origin.z(), -2.0);
}
