...
enum PARAMETER_TYPE { HEADER, POINT, ANALOG, FORCE_PLATFORM };
...
struct c3dTestStruct {
  ...
  float nSubframes = -1;
  float dummyForByteAlignment =
      -1; // Because of float precision, 4 bytes must be padded here due to the
          // odd numer of float variables
...
void fillC3D(c3dTestStruct &c3dStruc, bool withPoints, bool withAnalogs,
  ...
  if (withPoints) {
    ...
    pointRate.set(
        std::vector<double>() = {static_cast<double>(c3dStruc.pointFrameRate)});
    ...
    if (withAnalogs) {
      for (size_t sf = 0; sf < c3dStruc.nSubframes; ++sf) {
        ...
        for (size_t c = 0; c < c3dStruc.nAnalogs; ++c) {
          ...
          channel.data(static_cast<float>(2 * f + 3 * sf + 4 * c + 1) /
                       7.0f); // Generate random data
...
void defaultHeaderTest(const ezc3d::c3d &new_c3d,
                       HEADER_TYPE type = HEADER_TYPE::ALL) {
  ...
  if (type == HEADER_TYPE::POINT_ONLY ||
      ...
      type == HEADER_TYPE::POINT_AND_EVENT || type == HEADER_TYPE::ALL) {
  ...
  if (type == HEADER_TYPE::ANALOG_ONLY ||
      type == HEADER_TYPE::POINT_AND_ANALOG ||
      type == HEADER_TYPE::ANALOG_AND_EVENT || type == HEADER_TYPE::ALL) {
  ...
  // Event stuff
  if (type == HEADER_TYPE::EVENT_ONLY || type == HEADER_TYPE::POINT_AND_EVENT ||
      type == HEADER_TYPE::ANALOG_AND_EVENT || type == HEADER_TYPE::ALL) {
    ...
    EXPECT_THROW(
        new_c3d.header().eventsTime(new_c3d.header().eventsTime().size()),
        std::out_of_range);
    ...
    EXPECT_THROW(
        new_c3d.header().eventsLabel(new_c3d.header().eventsLabel().size()),
        std::out_of_range);
    ...
    EXPECT_THROW(
        new_c3d.header().eventsDisplay(new_c3d.header().eventsDisplay().size()),
        std::out_of_range);
  }
...
void compareHeader(const ezc3d::c3d &c3d1, const ezc3d::c3d &c3d2) {
  ...
  EXPECT_EQ(c3d1.header().nbAnalogsMeasurement(),
            c3d2.header().nbAnalogsMeasurement());
  ...
  for (size_t e = 0; e < c3d1.header().nbEvents(); ++e) {
    ...
    EXPECT_STREQ(c3d1.header().eventsLabel(e).c_str(),
                 c3d2.header().eventsLabel(e).c_str());
  }
}
  ...
  for (size_t f = 0; f < c3d1.header().nbFrames(); ++f) {
    for (size_t p = 0; p < c3d1.header().nb3dPoints(); ++p) {
      if (c3d1.data().frame(f).points().point(p).residual() < 0) {
        ...
        EXPECT_DOUBLE_AS_FLOAT_EQ(c3d1.data().frame(f).points().point(p).x(),
                                  c3d2.data().frame(f).points().point(p).x());
        EXPECT_DOUBLE_AS_FLOAT_EQ(c3d1.data().frame(f).points().point(p).y(),
                                  c3d2.data().frame(f).points().point(p).y());
        EXPECT_DOUBLE_AS_FLOAT_EQ(c3d1.data().frame(f).points().point(p).z(),
                                  c3d2.data().frame(f).points().point(p).z());
      }
      if (!skipResidual) {
        EXPECT_DOUBLE_AS_FLOAT_EQ(
            c3d1.data().frame(f).points().point(p).residual(),
            c3d2.data().frame(f).points().point(p).residual());
        std::vector<bool> cameraMasks1(
            c3d1.data().frame(f).points().point(p).cameraMask());
        std::vector<bool> cameraMasks2(
            c3d1.data().frame(f).points().point(p).cameraMask());
...
void defaultParametersTest(const ezc3d::c3d &new_c3d, PARAMETER_TYPE type) {
  if (type == PARAMETER_TYPE::HEADER) {
    ...
    EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("USED").type(),
              ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("POINT")
                  .parameter("USED")
                  .valuesAsInt()
                  .size(),
              1);
    EXPECT_EQ(
        new_c3d.parameters().group("POINT").parameter("USED").valuesAsInt()[0],
        0);
    EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("SCALE").type(),
              ezc3d::FLOAT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("POINT")
                  .parameter("SCALE")
                  ...
                  .size(),
              1);
    EXPECT_DOUBLE_AS_FLOAT_EQ(new_c3d.parameters()
                                  .group("POINT")
                                  .parameter("SCALE")
                                  .valuesAsDouble()[0],
                              -1);
    EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("RATE").type(),
              ezc3d::FLOAT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("POINT")
                  .parameter("RATE")
    ...
    EXPECT_DOUBLE_AS_FLOAT_EQ(new_c3d.parameters()
                                  ...
                                  .valuesAsDouble()[0],
                              0);
    // EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
    // 0); // ignore because it changes if analog is present
    EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("FRAMES").type(),
              ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("LABELS").type(),
              ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("POINT")
                  .parameter("LABELS")
              ...
              0);
    EXPECT_EQ(
        new_c3d.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
        ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("POINT")
                  ...
                  .size(),
              0);
    EXPECT_EQ(new_c3d.parameters().group("POINT").parameter("UNITS").type(),
              ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("POINT")
                  .parameter("UNITS")
    ...
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("USED").type(),
              ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("USED")
              ...
              1);
    EXPECT_EQ(
        new_c3d.parameters().group("ANALOG").parameter("USED").valuesAsInt()[0],
        0);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("LABELS").type(),
              ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("LABELS")
              ...
              0);
    EXPECT_EQ(
        new_c3d.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
        ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("DESCRIPTIONS")
              ...
              0);
    EXPECT_EQ(
        new_c3d.parameters().group("ANALOG").parameter("GEN_SCALE").type(),
        ezc3d::FLOAT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("GEN_SCALE")
    ...
    EXPECT_EQ(new_c3d.parameters()
                  ...
                  .valuesAsDouble()[0],
              1);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("SCALE").type(),
              ezc3d::FLOAT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("SCALE")
                  ...
                  .size(),
              0);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("OFFSET").type(),
              ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("OFFSET")
                  ...
                  .size(),
              0);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("UNITS").type(),
              ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("UNITS")
                  ...
                  .size(),
              0);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("RATE").type(),
              ezc3d::FLOAT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("RATE")
    ...
    EXPECT_DOUBLE_AS_FLOAT_EQ(new_c3d.parameters()
                                  ...
                                  .valuesAsDouble()[0],
                              0);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("FORMAT").type(),
              ezc3d::CHAR);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("FORMAT")
                  ...
                  .size(),
              0);
    EXPECT_EQ(new_c3d.parameters().group("ANALOG").parameter("BITS").type(),
              ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("ANALOG")
                  .parameter("BITS")
    ...
    EXPECT_EQ(
        new_c3d.parameters().group("FORCE_PLATFORM").parameter("USED").type(),
        ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
    ...
    EXPECT_EQ(new_c3d.parameters()
              ...
              0);
    EXPECT_EQ(
        new_c3d.parameters().group("FORCE_PLATFORM").parameter("TYPE").type(),
        ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("FORCE_PLATFORM")
                  .parameter("TYPE")
              ...
              0);
    EXPECT_EQ(
        new_c3d.parameters().group("FORCE_PLATFORM").parameter("ZERO").type(),
        ezc3d::INT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("FORCE_PLATFORM")
                  .parameter("ZERO")
    ...
    EXPECT_EQ(new_c3d.parameters()
              ...
              0);
    EXPECT_EQ(
        new_c3d.parameters().group("FORCE_PLATFORM").parameter("ORIGIN").type(),
        ezc3d::FLOAT);
    EXPECT_EQ(new_c3d.parameters()
                  .group("FORCE_PLATFORM")
                  .parameter("ORIGIN")
...
TEST(initialize, newC3D) {
  ...
  EXPECT_EQ(
      new_c3d.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
      0);
...
TEST(nonStandardC3d, zeroAndOneBasedFirstFrameValue) {
  ...
  // First frame is 1 (standard C3D)
  {
    ...
    int firstFrame = new_c3d.c3d.readInt(ezc3d::PROCESSOR_TYPE::INTEL, c3d_file,
                                         ezc3d::BYTE);
    c3d_file.seekg(firstFramePositionInFile + 2);
    int lastFrame = new_c3d.c3d.readInt(ezc3d::PROCESSOR_TYPE::INTEL, c3d_file,
                                        ezc3d::BYTE);
    c3d_file.close();
...
TEST(wrongC3D, wrongChecksumParameter) {
  ...
  c3d_file.seekp(
      static_cast<int>(256 * ezc3d::DATA_TYPE::WORD *
                           (new_c3d.header().parametersAddress() - 1) +
                       1)); // move to the parameter checksum
  int checksum(0x0);
  c3d_file.write(reinterpret_cast<const char *>(&checksum), ezc3d::BYTE);
  c3d_file.close();
...
TEST(parameters, getParametersAsInt) {
...

  {
    ...
    for (size_t i = 0; i < 3; ++i)
      EXPECT_THROW(static_cast<void>(p.valuesAsInt()[i]),
...

  {
    p.set(std::vector<std::string>(
        {"invalid_int1", "invalid_int2", "invalid_int3"}));
    for (size_t i = 0; i < 3; ++i)
      // Data are truncated
      EXPECT_THROW(static_cast<void>(p.valuesAsInt()[i]),
                   std::invalid_argument);
  }
  {
    ...
    for (size_t i = 0; i < 3; ++i)
      // Data are truncated
      EXPECT_THROW(static_cast<void>(p.valuesConvertedAsInt()[i]),
                   std::invalid_argument);
  }
}
...
TEST(parameters, getParametersAsDouble) {
...

  {
    p.set(std::vector<int>({1, 2, 3}));
    for (size_t i = 0; i < 3; ++i)
      EXPECT_THROW(static_cast<void>(p.valuesAsDouble()[i]),
                   std::invalid_argument);
  }
  {
    p.set(std::vector<int>({1, 2, 3}));
    for (size_t i = 0; i < 3; ++i)
      EXPECT_DOUBLE_AS_FLOAT_EQ(p.valuesConvertedAsDouble()[i],
...
TEST(c3dModifier, specificParameters) {
  ...
  EXPECT_THROW(new_c3d.c3d.parameters().group("ThisIsNotARealGroup"),
               std::invalid_argument);
  ...
  EXPECT_THROW(new_c3d.c3d.lockGroup("ThisIsNotARealGroup"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.unlockGroup("ThisIsNotARealGroup"),
               std::invalid_argument);
  ...
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ThisIsANewRealGroup")
                .parameter("NewParam")
                .type(),
            ezc3d::INT);
  EXPECT_EQ(new_c3d.c3d.parameters()
  ...
  EXPECT_THROW(new_c3d.c3d.parameters().group("POINT").parameter(nPointParams),
               std::out_of_range);
  EXPECT_THROW(new_c3d.c3d.parameters().group("POINT").parameterIdx(
                   "ThisIsNotARealParameter"),
               std::invalid_argument);

  // Get all the parameters
  const auto &params(
      new_c3d.c3d.parameters().group("ThisIsANewRealGroup").parameters());
...

  {
    ...
    pNonEmptyFloat.set(std::vector<double>({1.0}));
...
TEST(c3dModifier, removeGroup) {
  ...
  EXPECT_STREQ(new_c3d.c3d.parameters().group(3).name().c_str(),
               "existing_group");
  EXPECT_STREQ(new_c3d.c3d.parameters().group(4).name().c_str(),
               "removed_by_name_group");
  EXPECT_STREQ(new_c3d.c3d.parameters().group(5).name().c_str(),
               "ignored_group");
...
TEST(c3dModifier, removeParameter) {
  ...
  EXPECT_STREQ(
      new_c3d.c3d.parameters().group("new_group").parameter(0).name().c_str(),
      "existing_param");
  EXPECT_STREQ(
      new_c3d.c3d.parameters().group("new_group").parameter(1).name().c_str(),
      "removed_by_name");
  EXPECT_STREQ(
      new_c3d.c3d.parameters().group("new_group").parameter(2).name().c_str(),
      "ignored_param");
  ...
  EXPECT_THROW(
      new_c3d.c3d.remove("non_existing_group", "non_existing_parameter"),
      std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("new_group", "non_existing_parameter"),
               std::invalid_argument);
  EXPECT_EQ(new_c3d.c3d.parameters().group("new_group").nbParameters(), 2);
  EXPECT_STREQ(
      new_c3d.c3d.parameters().group("new_group").parameter(0).name().c_str(),
  ...
  EXPECT_EQ(new_c3d.c3d.parameters().group("new_group").nbParameters(), 2);
  EXPECT_STREQ(
      c3dBis.parameters().group("new_group").parameter(0).name().c_str(),
      "existing_param");
  EXPECT_STREQ(
      c3dBis.parameters().group("new_group").parameter(1).name().c_str(),
      "ignored_param");
  ...
  EXPECT_THROW(new_c3d.c3d.remove("POINT", "DESCRIPTIONS"),
               std::invalid_argument);
  ...
  EXPECT_THROW(new_c3d.c3d.remove("POINT", "DATA_START"),
               std::invalid_argument);
  ...
  EXPECT_THROW(new_c3d.c3d.remove("ANALOG", "DESCRIPTIONS"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("ANALOG", "GEN_SCALE"),
               std::invalid_argument);
  ...
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "USED"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "TYPE"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "CHANNEL"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "ZERO"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "ORIGIN"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "CORNERS"),
               std::invalid_argument);
  EXPECT_THROW(new_c3d.c3d.remove("FORCE_PLATFORM", "CAL_MATRIX"),
               std::invalid_argument);
...
TEST(c3dModifier, addPoints) {
  ...
  // Things that should have change
  EXPECT_EQ(new_c3d.c3d.parameters().group("POINT").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("USED")
  ...
  EXPECT_EQ(new_c3d.c3d.parameters()
                ...
                .valuesAsInt()[0],
            new_c3d.nPoints);
  EXPECT_EQ(new_c3d.c3d.parameters().group("POINT").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("SCALE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(new_c3d.c3d.parameters()
                                ...
                                .valuesAsDouble()[0],
                            -1);
  EXPECT_EQ(new_c3d.c3d.parameters().group("POINT").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(new_c3d.c3d.parameters()
                                ...
                                .parameter("RATE")
                                .valuesAsDouble()[0],
                            new_c3d.pointFrameRate);
  EXPECT_EQ(new_c3d.c3d.parameters().group("POINT").parameter("FRAMES").type(),
            ezc3d::INT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("FRAMES")
  ...
  EXPECT_EQ(new_c3d.c3d.parameters()
                ...
                .valuesAsInt()[0],
            new_c3d.nFrames);
  EXPECT_EQ(new_c3d.c3d.parameters().group("POINT").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("LABELS")
            ...
            new_c3d.nPoints);
  EXPECT_EQ(
      new_c3d.c3d.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            new_c3d.nPoints);
  EXPECT_EQ(new_c3d.c3d.parameters().group("POINT").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("POINT")
                .parameter("UNITS")
  ...
  for (size_t m = 0; m < new_c3d.nPoints; ++m) {
    EXPECT_STREQ(new_c3d.c3d.parameters()
                     .group("POINT")
                     .parameter("LABELS")
                     .valuesAsString()[m]
                     .c_str(),
                 new_c3d.pointNames[m].c_str());
    EXPECT_STREQ(new_c3d.c3d.parameters()
                     .group("POINT")
                     .parameter("DESCRIPTIONS")
                     .valuesAsString()[m]
  ...
  for (size_t f = 0; f < new_c3d.nFrames; ++f) {
    ...
    for (size_t m = 0; m < new_c3d.nPoints; ++m) {
      ...
      EXPECT_DOUBLE_EQ(new_c3d.c3d.data().frame(f).points().point(pointIdx).x(),
                       static_cast<double>(2 * f + 3 * m + 1) /
                           static_cast<double>(7.0));
      EXPECT_DOUBLE_EQ(new_c3d.c3d.data().frame(f).points().point(pointIdx).y(),
                       static_cast<double>(2 * f + 3 * m + 2) /
                           static_cast<double>(7.0));
      EXPECT_DOUBLE_EQ(new_c3d.c3d.data().frame(f).points().point(pointIdx).z(),
                       static_cast<double>(2 * f + 3 * m + 3) /
                           static_cast<double>(7.0));
      EXPECT_DOUBLE_EQ(
          new_c3d.c3d.data().frame(f).points().point(pointIdx).residual(), 0);
    }
  ...
  EXPECT_THROW(new_c3d.c3d.point("uselessPoint", new_frames),
               std::invalid_argument);
  ...
  for (size_t f = 0; f < new_c3d.c3d.data().nbFrames(); ++f) {
    ezc3d::DataNS::Points3dNS::Points pts;
    ezc3d::DataNS::Points3dNS::Point pt(
        new_c3d.c3d.data().frame(f).points().point(0));
    pts.point(pt);
    new_frames[f].add(pts);
  }
  EXPECT_THROW(new_c3d.c3d.point(new_c3d.pointNames, new_frames),
               std::invalid_argument);
...
TEST(c3dModifier, specificPoint) {
  ...
  for (size_t f = 0; f < new_c3d.nFrames; ++f) {
    for (size_t m = 0; m < new_c3d.nPoints; ++m) {
      size_t pointIdx(new_c3d.c3d.pointIdx(new_c3d.pointNames[m]));
      EXPECT_DOUBLE_EQ(new_c3d.c3d.data().frame(f).points().point(pointIdx).x(),
                       static_cast<double>(4 * f + 7 * m + 5) /
                           static_cast<double>(13.0));
      EXPECT_DOUBLE_EQ(new_c3d.c3d.data().frame(f).points().point(pointIdx).y(),
                       static_cast<double>(4 * f + 7 * m + 6) /
                           static_cast<double>(13.0));
      EXPECT_DOUBLE_EQ(new_c3d.c3d.data().frame(f).points().point(pointIdx).z(),
                       static_cast<double>(4 * f + 7 * m + 7) /
                           static_cast<double>(13.0));
      EXPECT_DOUBLE_EQ(
          new_c3d.c3d.data().frame(f).points().point(pointIdx).residual(), 0);
    }
  ...
  EXPECT_THROW(new_c3d.c3d.data().frame(0).points().point(new_c3d.nPoints),
               std::out_of_range);
...
TEST(c3dModifier, addAnalogs) {
  ...
  EXPECT_EQ(new_c3d.c3d.header().lastFrame(), new_c3d.nFrames - 1);
  EXPECT_EQ(new_c3d.c3d.header().nbFrames(), new_c3d.nFrames);
  EXPECT_EQ(new_c3d.c3d.header().nbAnalogsMeasurement(),
            new_c3d.nAnalogs * new_c3d.nSubframes);
  ...
  // Things that should have change
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("USED")
  ...
  EXPECT_EQ(new_c3d.c3d.parameters()
                ...
                .valuesAsInt()[0],
            new_c3d.nAnalogs);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("LABELS")
            ...
            new_c3d.nAnalogs);
  EXPECT_EQ(
      new_c3d.c3d.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("DESCRIPTIONS")
            ...
            new_c3d.nAnalogs);
  EXPECT_EQ(
      new_c3d.c3d.parameters().group("ANALOG").parameter("GEN_SCALE").type(),
      ezc3d::FLOAT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("GEN_SCALE")
  ...
  EXPECT_EQ(new_c3d.c3d.parameters()
                ...
                .valuesAsDouble()[0],
            1);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("SCALE")
                ...
                .size(),
            new_c3d.nAnalogs);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("OFFSET").type(),
            ezc3d::INT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("OFFSET")
                ...
                .size(),
            new_c3d.nAnalogs);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("UNITS")
                ...
                .size(),
            new_c3d.nAnalogs);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(new_c3d.c3d.parameters()
                                ...
                                .parameter("RATE")
                                .valuesAsDouble()[0],
                            new_c3d.analogFrameRate);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("FORMAT").type(),
            ezc3d::CHAR);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("FORMAT")
                ...
                .size(),
            0);
  EXPECT_EQ(new_c3d.c3d.parameters().group("ANALOG").parameter("BITS").type(),
            ezc3d::INT);
  EXPECT_EQ(new_c3d.c3d.parameters()
                .group("ANALOG")
                .parameter("BITS")
      ...
      for (size_t c = 0; c < new_c3d.nAnalogs; ++c)
        EXPECT_DOUBLE_AS_FLOAT_EQ(
            new_c3d.c3d.data()
                ...
                .channel(c)
                .data(),
            static_cast<double>(2 * f + 3 * sf + 4 * c + 1) /
                static_cast<double>(7.0));
  }
...
TEST(c3dModifier, specificAnalog) {
  ...
  EXPECT_STREQ(new_c3d.c3d.parameters()
                   .group("ANALOG")
                   .parameter("LABELS")
                   .valuesAsString()[new_c3d.nAnalogs - 1]
                   .c_str(),
               "AnalogNameWithSpaceAtTheEnd");

  // Add analog by frames
  std::vector<ezc3d::DataNS::Frame> frames;
  // Wrong number of frames
  EXPECT_THROW(new_c3d.c3d.analog("uselessChannel", frames),
               std::invalid_argument);
  ...
  EXPECT_THROW(new_c3d.c3d.analog("wrongChannel", frames),
               std::invalid_argument);
  ...
  for (size_t f = 0; f < new_c3d.nFrames; ++f) {
    ...
    for (size_t sf = 0; sf < new_c3d.nSubframes; ++sf) {
      ...
      for (size_t c = 0; c < new_c3d.nAnalogs; ++c) {
        ezc3d::DataNS::AnalogsNS::Channel channel;
        channel.data(static_cast<double>(2 * f + 3 * sf + 4 * c + 1) /
                     static_cast<double>(7.0)); // Generate random data
        subframes.channel(channel);
      }
      analogs.subframe(subframes);
    }
  ...
  EXPECT_THROW(new_c3d.c3d.analog(new_c3d.analogNames, frames),
               std::invalid_argument);

  // No throw
  std::vector<std::string> analogNames = {"NewAnalog1", "NewAnalog2",
                                          "NewAnalog3", "NewAnalog4"};
  for (size_t f = 0; f < new_c3d.nFrames; ++f) {
    ezc3d::DataNS::Frame frame;
    ezc3d::DataNS::AnalogsNS::Analogs analogs;
    for (size_t sf = 0; sf < new_c3d.nSubframes; ++sf) {
  ...
  // Get outside channel
  EXPECT_THROW(new_c3d.c3d.data().frame(0).analogs().subframe(0).channel(8),
               std::out_of_range);
  ...
  for (size_t c = 0; c < new_c3d.analogNames.size(); ++c) {
    ...
    EXPECT_NO_THROW(
        new_c3d.c3d.data().frame(0).analogs().subframe(0).channel(channelName));
  }
  EXPECT_THROW(new_c3d.c3d.channelIdx("ThisIsNotARealChannel"),
               std::invalid_argument);

  // Get a subframe
  {
    EXPECT_THROW(new_c3d.c3d.data().frame(new_c3d.c3d.data().nbFrames()),
                 std::out_of_range);
  ...
  // adding/replacing channels and getting them
  {
    ...
    EXPECT_NO_THROW(
        subframe.channel(channelToReplace, subframe.nbChannels() + 2));
...
TEST(c3dModifier, addFrames) {
  ...
  // Wrong number of frames
  ezc3d::DataNS::Points3dNS::Points stupidPoints(
      new_c3d.c3d.data().frame(0).points());
  stupidFramePoint.add(stupidPoints);
  new_c3d.c3d.frame(stupidFramePoint);

  // Add Analogs
  new_c3d.nAnalogs = 3;
  ezc3d::ParametersNS::GroupNS::Parameter analogUsed(
      new_c3d.c3d.parameters().group("ANALOG").parameter("USED"));
  ...
  ezc3d::DataNS::AnalogsNS::Analogs stupidAnalogs(
      new_c3d.c3d.data().frame(0).analogs());
  ...
  EXPECT_THROW(new_c3d.c3d.frame(stupidFrameAnalog),
               std::runtime_error); // Wrong frame rate for analogs

  ezc3d::ParametersNS::GroupNS::Parameter analogRate(
      new_c3d.c3d.parameters().group("ANALOG").parameter("RATE"));
  ...
  ezc3d::ParametersNS::GroupNS::Parameter pointRate(
      new_c3d.c3d.parameters().group("POINT").parameter("RATE"));
...
TEST(c3dModifier, specificFrames) {
  ...
  EXPECT_THROW(new_c3d.c3d.data().frame(0).analogs().subframe(0),
               std::out_of_range);
  ...
  for (size_t f = 0; f < new_c3d.nFrames; ++f) {
    if (f != new_c3d.nFrames - 2) { // Where no frames where added
      for (size_t m = 0; m < new_c3d.nPoints; ++m) {
        size_t pointIdx(new_c3d.c3d.pointIdx(new_c3d.pointNames[m]));
        EXPECT_DOUBLE_EQ(
            new_c3d.c3d.data().frame(f).points().point(pointIdx).x(),
            static_cast<double>(4 * f + 2 * m + 5) / static_cast<double>(17.0));
        EXPECT_DOUBLE_EQ(
            new_c3d.c3d.data().frame(f).points().point(pointIdx).y(),
            static_cast<double>(4 * f + 2 * m + 6) / static_cast<double>(17.0));
        EXPECT_DOUBLE_EQ(
            new_c3d.c3d.data().frame(f).points().point(pointIdx).z(),
            static_cast<double>(4 * f + 2 * m + 7) / static_cast<double>(17.0));
      }
    } else {
      for (size_t m = 0; m < new_c3d.nPoints; ++m) {
        size_t pointIdx(new_c3d.c3d.pointIdx(new_c3d.pointNames[m]));
        EXPECT_THROW(new_c3d.c3d.data().frame(f).points().point(pointIdx).x(),
                     std::out_of_range);
        EXPECT_THROW(new_c3d.c3d.data().frame(f).points().point(pointIdx).y(),
                     std::out_of_range);
        EXPECT_THROW(new_c3d.c3d.data().frame(f).points().point(pointIdx).z(),
                     std::out_of_range);
      }
    }
  }
...
TEST(c3dFileIO, CreateWriteAndReadBack) {
  ...
  EXPECT_EQ(read_c3d.header().nbAnalogsMeasurement(),
            ref_c3d.nAnalogs * ref_c3d.nSubframes);
  ...
  // Things that should have change
  EXPECT_EQ(read_c3d.parameters().group("POINT").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("USED")
                .valuesAsInt()
                .size(),
            1);
  EXPECT_EQ(
      read_c3d.parameters().group("POINT").parameter("USED").valuesAsInt()[0],
      ref_c3d.nPoints);
  EXPECT_EQ(read_c3d.parameters().group("POINT").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("SCALE")
                ...
                .size(),
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(read_c3d.parameters()
                                .group("POINT")
                                .parameter("SCALE")
                                .valuesAsDouble()[0],
                            -1);
  EXPECT_EQ(read_c3d.parameters().group("POINT").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(read_c3d.parameters()
                                ...
                                .parameter("RATE")
                                .valuesAsDouble()[0],
                            ref_c3d.pointFrameRate);
  EXPECT_EQ(read_c3d.parameters().group("POINT").parameter("FRAMES").type(),
            ezc3d::INT);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("FRAMES")
            ...
            1);
  EXPECT_EQ(
      read_c3d.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
      ref_c3d.nFrames);
  EXPECT_EQ(read_c3d.parameters().group("POINT").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("LABELS")
            ...
            ref_c3d.nPoints);
  EXPECT_EQ(
      read_c3d.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            ref_c3d.nPoints);
  EXPECT_EQ(read_c3d.parameters().group("POINT").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("POINT")
                .parameter("UNITS")
  ...
  for (size_t m = 0; m < ref_c3d.nPoints; ++m) {
    EXPECT_STREQ(read_c3d.parameters()
                     .group("POINT")
                     .parameter("LABELS")
                     .valuesAsString()[m]
                     .c_str(),
                 ref_c3d.pointNames[m].c_str());
    EXPECT_STREQ(read_c3d.parameters()
                     .group("POINT")
                     .parameter("DESCRIPTIONS")
                     .valuesAsString()[m]
  ...
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("USED")
            ...
            1);
  EXPECT_EQ(
      read_c3d.parameters().group("ANALOG").parameter("USED").valuesAsInt()[0],
      ref_c3d.nAnalogs);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("LABELS")
            ...
            ref_c3d.nAnalogs);
  EXPECT_EQ(
      read_c3d.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            ref_c3d.nAnalogs);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("GEN_SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("GEN_SCALE")
  ...
  EXPECT_EQ(read_c3d.parameters()
                ...
                .valuesAsDouble()[0],
            1);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("SCALE")
                ...
                .size(),
            ref_c3d.nAnalogs);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("OFFSET").type(),
            ezc3d::INT);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("OFFSET")
                ...
                .size(),
            ref_c3d.nAnalogs);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("UNITS")
                ...
                .size(),
            ref_c3d.nAnalogs);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(read_c3d.parameters()
                                ...
                                .parameter("RATE")
                                .valuesAsDouble()[0],
                            ref_c3d.analogFrameRate);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("FORMAT").type(),
            ezc3d::CHAR);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("FORMAT")
                ...
                .size(),
            0);
  EXPECT_EQ(read_c3d.parameters().group("ANALOG").parameter("BITS").type(),
            ezc3d::INT);
  EXPECT_EQ(read_c3d.parameters()
                .group("ANALOG")
                .parameter("BITS")
  ...
  for (size_t f = 0; f < ref_c3d.nFrames; ++f) {
    for (size_t m = 0; m < ref_c3d.nPoints; ++m) {
      ...
      EXPECT_DOUBLE_AS_FLOAT_EQ(
          read_c3d.data().frame(f).points().point(pointIdx).x(),
          static_cast<double>(2 * f + 3 * m + 1) / static_cast<double>(7.0));
      EXPECT_DOUBLE_AS_FLOAT_EQ(
          read_c3d.data().frame(f).points().point(pointIdx).y(),
          static_cast<double>(2 * f + 3 * m + 2) / static_cast<double>(7.0));
      EXPECT_DOUBLE_AS_FLOAT_EQ(
          read_c3d.data().frame(f).points().point(pointIdx).z(),
          static_cast<double>(2 * f + 3 * m + 3) / static_cast<double>(7.0));
    }

    for (size_t sf = 0; sf < ref_c3d.nSubframes; ++sf)
      for (size_t c = 0; c < ref_c3d.nAnalogs; ++c)
        EXPECT_DOUBLE_AS_FLOAT_EQ(
            read_c3d.data().frame(f).analogs().subframe(sf).channel(c).data(),
            static_cast<double>(2 * f + 3 * sf + 4 * c + 1) /
                static_cast<double>(7.0));
  }
}
...
TEST(c3dFileIO, CreateWriteAndReadBackWithNan) {
  ...
  ezc3d::DataNS::AnalogsNS::Channel channel(read_c3d.data()
                                                .frame(idxFrame)
                                                .analogs()
                                                .subframe(idxSubframe)
                                                .channel(idxChannel));
...
TEST(c3dFileIO, readC3DWithRotation) {
  ...
  EXPECT_EQ(c3d.parameters().group("ROTATION").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(
      c3d.parameters().group("ROTATION").parameter("USED").valuesAsInt().size(),
      1);
  EXPECT_EQ(
      c3d.parameters().group("ROTATION").parameter("USED").valuesAsInt()[0],
      21);
  EXPECT_EQ(c3d.parameters().group("ROTATION").parameter("DATA_START").type(),
            ezc3d::INT);
  EXPECT_EQ(c3d.parameters()
                .group("ROTATION")
  ...
  EXPECT_EQ(c3d.parameters()
                ...
                .valuesAsInt()[0],
            6);
  EXPECT_EQ(c3d.parameters().group("ROTATION").parameter("RATIO").type(),
            ezc3d::INT);
  EXPECT_EQ(c3d.parameters()
                .group("ROTATION")
                .parameter("RATIO")
            ...
            1);
  EXPECT_EQ(
      c3d.parameters().group("ROTATION").parameter("RATIO").valuesAsInt()[0],
      1);
  EXPECT_EQ(c3d.parameters().group("ROTATION").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(c3d.parameters()
                .group("ROTATION")
                .parameter("LABELS")
                ...
                .size(),
            21);
  EXPECT_EQ(c3d.parameters().group("ROTATION").parameter("DESCRIPTIONS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(c3d.parameters()
                .group("ROTATION")
                .parameter("DESCRIPTIONS")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      c3d.data().frame(5).rotations().subframe(0).rotation(2)(2, 3), 931.63824);
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(sumValues, 9367047.137371358);
  ...
  EXPECT_EQ(c3dCopy.parameters().group("ROTATION").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(c3dCopy.parameters()
                .group("ROTATION")
                .parameter("USED")
                .valuesAsInt()
                .size(),
            1);
  EXPECT_EQ(
      c3dCopy.parameters().group("ROTATION").parameter("USED").valuesAsInt()[0],
      21);
  EXPECT_EQ(
      c3dCopy.parameters().group("ROTATION").parameter("DATA_START").type(),
      ezc3d::INT);
  EXPECT_EQ(c3dCopy.parameters()
                .group("ROTATION")
                .parameter("DATA_START")
  ...
  EXPECT_EQ(c3dCopy.parameters()
                ...
                .valuesAsInt()[0],
            7);
  EXPECT_EQ(c3dCopy.parameters().group("ROTATION").parameter("RATIO").type(),
            ezc3d::INT);
  EXPECT_EQ(c3dCopy.parameters()
                .group("ROTATION")
                .parameter("RATIO")
  ...
  EXPECT_EQ(c3dCopy.parameters()
                ...
                .valuesAsInt()[0],
            1);
  EXPECT_EQ(c3dCopy.parameters().group("ROTATION").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(c3dCopy.parameters()
                .group("ROTATION")
                .parameter("LABELS")
            ...
            21);
  EXPECT_EQ(
      c3dCopy.parameters().group("ROTATION").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(c3dCopy.parameters()
                .group("ROTATION")
                .parameter("DESCRIPTIONS")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(sumValues, sumValuesCopy);
}
...
TEST(c3dFileIO, readViconC3D) {
  ...
  EXPECT_THROW(Vicon.header().eventsTime(Vicon.header().eventsTime().size()),
               std::out_of_range);
  ...
  EXPECT_THROW(Vicon.header().eventsLabel(Vicon.header().eventsLabel().size()),
               std::out_of_range);
  ...
  EXPECT_THROW(
      Vicon.header().eventsDisplay(Vicon.header().eventsDisplay().size()),
      std::out_of_range);
  ...
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(
      Vicon.parameters().group("POINT").parameter("USED").valuesAsInt().size(),
      1);
  EXPECT_EQ(
      Vicon.parameters().group("POINT").parameter("USED").valuesAsInt()[0], 51);
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("POINT")
                .parameter("SCALE")
                .valuesAsDouble()
                .size(),
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      Vicon.parameters().group("POINT").parameter("SCALE").valuesAsDouble()[0],
      -0.0099999998f);
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("POINT")
                .parameter("RATE")
            ...
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      Vicon.parameters().group("POINT").parameter("RATE").valuesAsDouble()[0],
      100);
  EXPECT_EQ(
      Vicon.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
      580); // ignore because it changes if analog is present
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("FRAMES").type(),
            ezc3d::INT);
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Vicon.parameters()
                .group("POINT")
                .parameter("LABELS")
                ...
                .size(),
            51);
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Vicon.parameters()
                .group("POINT")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            51);
  EXPECT_EQ(Vicon.parameters().group("POINT").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Vicon.parameters()
                .group("POINT")
                .parameter("UNITS")
  ...
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(
      Vicon.parameters().group("ANALOG").parameter("USED").valuesAsInt().size(),
      1);
  EXPECT_EQ(
      Vicon.parameters().group("ANALOG").parameter("USED").valuesAsInt()[0],
      38);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("LABELS")
                ...
                .size(),
            38);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            38);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("GEN_SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("GEN_SCALE")
  ...
  EXPECT_EQ(Vicon.parameters()
                ...
                .valuesAsDouble()[0],
            1);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("SCALE")
                ...
                .size(),
            38);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("OFFSET").type(),
            ezc3d::INT);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("OFFSET")
                ...
                .size(),
            38);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("UNITS")
                ...
                .size(),
            38);
  EXPECT_EQ(Vicon.parameters().group("ANALOG").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("ANALOG")
                .parameter("RATE")
            ...
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      Vicon.parameters().group("ANALOG").parameter("RATE").valuesAsDouble()[0],
      2000);

  EXPECT_EQ(Vicon.parameters().group("FORCE_PLATFORM").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(Vicon.parameters()
                .group("FORCE_PLATFORM")
                .parameter("USED")
  ...
  EXPECT_EQ(Vicon.parameters()
                ...
                .valuesAsInt()[0],
            0);
  EXPECT_EQ(Vicon.parameters().group("FORCE_PLATFORM").parameter("TYPE").type(),
            ezc3d::INT);
  EXPECT_EQ(Vicon.parameters()
                .group("FORCE_PLATFORM")
                .parameter("TYPE")
                ...
                .size(),
            0);
  EXPECT_EQ(Vicon.parameters().group("FORCE_PLATFORM").parameter("ZERO").type(),
            ezc3d::INT);
  EXPECT_EQ(Vicon.parameters()
                .group("FORCE_PLATFORM")
                .parameter("ZERO")
  ...
  EXPECT_EQ(Vicon.parameters()
            ...
            0);
  EXPECT_EQ(
      Vicon.parameters().group("FORCE_PLATFORM").parameter("CORNERS").type(),
      ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("FORCE_PLATFORM")
                .parameter("CORNERS")
            ...
            0);
  EXPECT_EQ(
      Vicon.parameters().group("FORCE_PLATFORM").parameter("ORIGIN").type(),
      ezc3d::FLOAT);
  EXPECT_EQ(Vicon.parameters()
                .group("FORCE_PLATFORM")
                .parameter("ORIGIN")
            ...
            0);
  EXPECT_EQ(
      Vicon.parameters().group("FORCE_PLATFORM").parameter("CHANNEL").type(),
      ezc3d::INT);
  EXPECT_EQ(Vicon.parameters()
                .group("FORCE_PLATFORM")
                .parameter("CHANNEL")
  ...
  // Test some values randomly
  EXPECT_DOUBLE_AS_FLOAT_EQ(Vicon.data().frame(0).points().point(0).x(),
                            44.16278839111328);
  EXPECT_DOUBLE_AS_FLOAT_EQ(Vicon.data()
                                .frame(Vicon.data().frames().size() - 1)
                                .points()
                                .point(50)
                                .z(),
                            99.682586669921875);
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(sumValues, 42506014.918278672);
}
...
TEST(c3dFileIO, readQualisysC3D) {
  ...
  EXPECT_THROW(
      Qualisys.header().eventsTime(Qualisys.header().eventsTime().size()),
      std::out_of_range);
  ...
  EXPECT_THROW(
      Qualisys.header().eventsLabel(Qualisys.header().eventsLabel().size()),
      std::out_of_range);
  ...
  EXPECT_THROW(
      Qualisys.header().eventsDisplay(Qualisys.header().eventsDisplay().size()),
      std::out_of_range);
  ...
  EXPECT_EQ(Qualisys.parameters().group("POINT").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("POINT")
                .parameter("USED")
                .valuesAsInt()
                .size(),
            1);
  EXPECT_EQ(
      Qualisys.parameters().group("POINT").parameter("USED").valuesAsInt()[0],
      55);
  EXPECT_EQ(Qualisys.parameters().group("POINT").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("POINT")
                .parameter("SCALE")
                ...
                .size(),
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(Qualisys.parameters()
                                .group("POINT")
                                .parameter("SCALE")
                                .valuesAsDouble()[0],
                            -0.076232255f);
  EXPECT_EQ(Qualisys.parameters().group("POINT").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("POINT")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(Qualisys.parameters()
                            ...
                            200);
  EXPECT_EQ(
      Qualisys.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
      340); // ignore because it changes if analog is present
  EXPECT_EQ(Qualisys.parameters().group("POINT").parameter("FRAMES").type(),
            ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters().group("POINT").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Qualisys.parameters()
                .group("POINT")
                .parameter("LABELS")
            ...
            55);
  EXPECT_EQ(
      Qualisys.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(Qualisys.parameters()
                .group("POINT")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            55);
  EXPECT_EQ(Qualisys.parameters().group("POINT").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Qualisys.parameters()
                .group("POINT")
                .parameter("UNITS")
  ...
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("USED")
            ...
            1);
  EXPECT_EQ(
      Qualisys.parameters().group("ANALOG").parameter("USED").valuesAsInt()[0],
      69);
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("LABELS")
            ...
            69);
  EXPECT_EQ(
      Qualisys.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            69);
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("GEN_SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("GEN_SCALE")
  ...
  EXPECT_EQ(Qualisys.parameters()
                ...
                .valuesAsDouble()[0],
            1);
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("SCALE")
                ...
                .size(),
            69);
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("OFFSET").type(),
            ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("OFFSET")
                ...
                .size(),
            69);
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("UNITS")
                ...
                .size(),
            69);
  EXPECT_EQ(Qualisys.parameters().group("ANALOG").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("ANALOG")
                .parameter("RATE")
  ...
  EXPECT_EQ(
      Qualisys.parameters().group("FORCE_PLATFORM").parameter("USED").type(),
      ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("FORCE_PLATFORM")
                .parameter("USED")
  ...
  EXPECT_EQ(Qualisys.parameters()
            ...
            2);
  EXPECT_EQ(
      Qualisys.parameters().group("FORCE_PLATFORM").parameter("TYPE").type(),
      ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("FORCE_PLATFORM")
                .parameter("TYPE")
            ...
            2);
  EXPECT_EQ(
      Qualisys.parameters().group("FORCE_PLATFORM").parameter("ZERO").type(),
      ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("FORCE_PLATFORM")
                .parameter("ZERO")
  ...
  EXPECT_EQ(Qualisys.parameters()
            ...
            0);
  EXPECT_EQ(
      Qualisys.parameters().group("FORCE_PLATFORM").parameter("CORNERS").type(),
      ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("FORCE_PLATFORM")
                .parameter("CORNERS")
            ...
            24);
  EXPECT_EQ(
      Qualisys.parameters().group("FORCE_PLATFORM").parameter("ORIGIN").type(),
      ezc3d::FLOAT);
  EXPECT_EQ(Qualisys.parameters()
                .group("FORCE_PLATFORM")
                .parameter("ORIGIN")
            ...
            6);
  EXPECT_EQ(
      Qualisys.parameters().group("FORCE_PLATFORM").parameter("CHANNEL").type(),
      ezc3d::INT);
  EXPECT_EQ(Qualisys.parameters()
                .group("FORCE_PLATFORM")
                .parameter("CHANNEL")
  ...
  EXPECT_STREQ(Qualisys.parameters()
                   .group("MANUFACTURER")
                   .parameter("COMPANY")
                   .valuesAsString()[0]
                   .c_str(),
  ...
  for (size_t f = 0; f < 340; ++f) {
    ...
    for (size_t sf = 0; sf < 10; ++sf)
      EXPECT_EQ(Qualisys.data().frame(f).analogs().subframe(sf).nbChannels(),
                69);
  }
}
...
TEST(c3dFileIO, readOptotrakC3D) {
  ...
  EXPECT_THROW(
      Optotrak.header().eventsTime(Optotrak.header().eventsTime().size()),
      std::out_of_range);
  ...
  EXPECT_THROW(
      Optotrak.header().eventsLabel(Optotrak.header().eventsLabel().size()),
      std::out_of_range);
  ...
  EXPECT_THROW(
      Optotrak.header().eventsDisplay(Optotrak.header().eventsDisplay().size()),
      std::out_of_range);
  ...
  EXPECT_EQ(Optotrak.parameters().group("POINT").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(Optotrak.parameters()
                .group("POINT")
                .parameter("USED")
                .valuesAsInt()
                .size(),
            1);
  EXPECT_EQ(
      Optotrak.parameters().group("POINT").parameter("USED").valuesAsInt()[0],
      54);
  EXPECT_EQ(Optotrak.parameters().group("POINT").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Optotrak.parameters()
                .group("POINT")
                .parameter("SCALE")
                ...
                .size(),
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(Optotrak.parameters()
                                .group("POINT")
                                .parameter("SCALE")
                                .valuesAsDouble()[0],
                            -7.8661418f);
  EXPECT_EQ(Optotrak.parameters().group("POINT").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Optotrak.parameters()
                .group("POINT")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(Optotrak.parameters()
                            ...
                            30);
  EXPECT_EQ(
      Optotrak.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
      29); // ignore because it changes if analog is present
  EXPECT_EQ(Optotrak.parameters().group("POINT").parameter("FRAMES").type(),
            ezc3d::INT);
  EXPECT_EQ(Optotrak.parameters().group("POINT").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Optotrak.parameters()
                .group("POINT")
                .parameter("LABELS")
            ...
            54);
  EXPECT_EQ(
      Optotrak.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(Optotrak.parameters()
                .group("POINT")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            54);
  EXPECT_EQ(Optotrak.parameters().group("POINT").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Optotrak.parameters()
                .group("POINT")
                .parameter("UNITS")
...
TEST(c3dFileIO, readKistlerC3D) {
  ...
  EXPECT_EQ(Optotrak.parameters().group("ANALOG").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(Optotrak.parameters()
                .group("ANALOG")
                .parameter("USED")
            ...
            1);
  EXPECT_EQ(
      Optotrak.parameters().group("ANALOG").parameter("USED").valuesAsInt()[0],
      16);
  EXPECT_EQ(Optotrak.parameters().group("ANALOG").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Optotrak.parameters()
                .group("ANALOG")
                .parameter("SCALE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(Optotrak.parameters()
                                ...
                                .valuesAsDouble()[0],
                            0.038985152f);
  EXPECT_EQ(Optotrak.parameters().group("ANALOG").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(Optotrak.parameters()
                .group("ANALOG")
                .parameter("RATE")
  ...
  EXPECT_DOUBLE_AS_FLOAT_EQ(Optotrak.parameters()
                                ...
                                .valuesAsDouble()[0],
                            600);
  EXPECT_EQ(Optotrak.parameters().group("ANALOG").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Optotrak.parameters()
                .group("ANALOG")
                .parameter("LABELS")
            ...
            16);
  EXPECT_EQ(
      Optotrak.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
      ezc3d::CHAR);
  EXPECT_EQ(Optotrak.parameters()
                .group("ANALOG")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            16);
  EXPECT_EQ(Optotrak.parameters().group("ANALOG").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(Optotrak.parameters()
                .group("ANALOG")
                .parameter("UNITS")
  ...
  for (size_t f = 0; f < 30; ++f) {
    ...
    for (size_t sf = 0; sf < Optotrak.data().frame(f).analogs().nbSubframes();
         sf++) {
      EXPECT_EQ(Optotrak.data().frame(f).analogs().subframe(sf).nbChannels(),
                16);
    }
  }
}
...
TEST(c3dFileio, readBtsC3D) {
  ...
  EXPECT_THROW(BTS.header().eventsTime(BTS.header().eventsTime().size()),
               std::out_of_range);
  ...
  EXPECT_THROW(BTS.header().eventsLabel(BTS.header().eventsLabel().size()),
               std::out_of_range);
  ...
  EXPECT_THROW(BTS.header().eventsDisplay(BTS.header().eventsDisplay().size()),
               std::out_of_range);
  ...
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(
      BTS.parameters().group("POINT").parameter("USED").valuesAsInt().size(),
      1);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("USED").valuesAsInt()[0],
            22);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(BTS.parameters()
                .group("POINT")
                .parameter("SCALE")
                .valuesAsDouble()
                .size(),
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      BTS.parameters().group("POINT").parameter("SCALE").valuesAsDouble()[0],
      -0.1f);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(
      BTS.parameters().group("POINT").parameter("RATE").valuesAsDouble().size(),
      1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      BTS.parameters().group("POINT").parameter("RATE").valuesAsDouble()[0],
      100);
  EXPECT_EQ(
      BTS.parameters().group("POINT").parameter("FRAMES").valuesAsInt()[0],
      675);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("FRAMES").type(),
            ezc3d::INT);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(BTS.parameters()
                .group("POINT")
                .parameter("LABELS")
                ...
                .size(),
            22);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("DESCRIPTIONS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(BTS.parameters()
                .group("POINT")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            22);
  EXPECT_EQ(BTS.parameters().group("POINT").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(BTS.parameters()
                .group("POINT")
                .parameter("UNITS")
                .valuesAsString()
                .size(),
            1); // This might be weird. Shouldn't there be 22 as well?

  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(
      BTS.parameters().group("ANALOG").parameter("USED").valuesAsInt().size(),
      1);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("USED").valuesAsInt()[0],
            44);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("LABELS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(BTS.parameters()
                .group("ANALOG")
                .parameter("LABELS")
                ...
                .size(),
            44);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("DESCRIPTIONS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(BTS.parameters()
                .group("ANALOG")
                .parameter("DESCRIPTIONS")
                ...
                .size(),
            44);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("GEN_SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(BTS.parameters()
                .group("ANALOG")
                .parameter("GEN_SCALE")
  ...
  EXPECT_EQ(BTS.parameters()
                ...
                .valuesAsDouble()[0],
            1);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("SCALE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(BTS.parameters()
                .group("ANALOG")
                .parameter("SCALE")
                ...
                .size(),
            44);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("OFFSET").type(),
            ezc3d::INT);
  EXPECT_EQ(
      BTS.parameters().group("ANALOG").parameter("OFFSET").valuesAsInt().size(),
      44);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("UNITS").type(),
            ezc3d::CHAR);
  EXPECT_EQ(BTS.parameters()
                .group("ANALOG")
                .parameter("UNITS")
                ...
                .size(),
            44);
  EXPECT_EQ(BTS.parameters().group("ANALOG").parameter("RATE").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(BTS.parameters()
                .group("ANALOG")
                .parameter("RATE")
            ...
            1);
  EXPECT_DOUBLE_AS_FLOAT_EQ(
      BTS.parameters().group("ANALOG").parameter("RATE").valuesAsDouble()[0],
      1000);

  EXPECT_EQ(BTS.parameters().group("FORCE_PLATFORM").parameter("USED").type(),
            ezc3d::INT);
  EXPECT_EQ(BTS.parameters()
                .group("FORCE_PLATFORM")
                .parameter("USED")
  ...
  EXPECT_EQ(BTS.parameters()
                ...
                .valuesAsInt()[0],
            6);
  EXPECT_EQ(BTS.parameters().group("FORCE_PLATFORM").parameter("TYPE").type(),
            ezc3d::INT);
  EXPECT_EQ(BTS.parameters()
                .group("FORCE_PLATFORM")
                .parameter("TYPE")
                ...
                .size(),
            6);
  EXPECT_EQ(BTS.parameters().group("FORCE_PLATFORM").parameter("ZERO").type(),
            ezc3d::INT);
  EXPECT_EQ(BTS.parameters()
                .group("FORCE_PLATFORM")
                .parameter("ZERO")
  ...
  EXPECT_EQ(BTS.parameters()
            ...
            0);
  EXPECT_EQ(
      BTS.parameters().group("FORCE_PLATFORM").parameter("CORNERS").type(),
      ezc3d::FLOAT);
  EXPECT_EQ(BTS.parameters()
                .group("FORCE_PLATFORM")
                .parameter("CORNERS")
                ...
                .size(),
            72);
  EXPECT_EQ(BTS.parameters().group("FORCE_PLATFORM").parameter("ORIGIN").type(),
            ezc3d::FLOAT);
  EXPECT_EQ(BTS.parameters()
                .group("FORCE_PLATFORM")
                .parameter("ORIGIN")
            ...
            18);
  EXPECT_EQ(
      BTS.parameters().group("FORCE_PLATFORM").parameter("CHANNEL").type(),
      ezc3d::INT);
  EXPECT_EQ(BTS.parameters()
                .group("FORCE_PLATFORM")
                .parameter("CHANNEL")
...
TEST(c3dFileIO, comparedIdenticalFilesSample1) {
  ...
  EXPECT_THROW(ezc3d::c3d c3d_sr("c3dTestFiles/Eb015dr.c3d"),
               std::runtime_error); // MIPS float format
  EXPECT_THROW(ezc3d::c3d c3d_si("c3dTestFiles/Eb015di.c3d"),
               std::runtime_error); // MIPS integer format
...
TEST(c3dFileIO, comparedIdenticalFilesSample2) {
  ...
  EXPECT_THROW(ezc3d::c3d c3d_sr("c3dTestFiles/sgi_real.c3d"),
               std::runtime_error); // MIPS float format
  EXPECT_THROW(ezc3d::c3d c3d_si("c3dTestFiles/sgi_int.c3d"),
               std::runtime_error); // MIPS integer format
