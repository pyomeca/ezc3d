...
int main()
{
    ...
    ezC3D::DataNS::Points3d::Point p(c3d.data().frame(10).points()->point(namePoint));
    p.print();

    // Test for specific subframe Analog
    std::string nameAnalog(c3d.data().frame(10).analogs()->subframe(2).channel(2).name());
    ezC3D::DataNS::Analogs::SubFrame::Channel c(c3d.data().frame(10).analogs()->subframe(2).channel(nameAnalog));
    c.print();
