...
ezc3d::Matrix::Matrix()
    : _nbRows(0), _nbCols(0), _data(std::vector<double>()) {}

ezc3d::Matrix::Matrix(size_t nbRows, size_t nbCols)
    : _nbRows(nbRows), _nbCols(nbCols),
      _data(std::vector<double>(nbRows * nbCols)) {}

ezc3d::Matrix::Matrix(const ezc3d::Matrix &matrix)
    : _nbRows(matrix._nbRows), _nbCols(matrix._nbCols), _data(matrix._data) {}

ezc3d::Matrix::Matrix(const std::vector<Vector3d> &matrix)
    : _nbRows(3), _nbCols(matrix.size()),
...
ezc3d::Matrix::Matrix(const std::vector<Vector6d> &matrix)
    : _nbRows(6), _nbCols(matrix.size()),
      _data(std::vector<double>(_nbRows * _nbCols)) {
  for (size_t i = 0; i < _nbCols; ++i) {
    for (size_t j = 0; j < _nbRows; ++j) {
      _data[i * _nbRows + j] = matrix[i](j);
...
size_t ezc3d::Matrix::size() const { return _data.size(); }

size_t ezc3d::Matrix::nbRows() const { return _nbRows; }

size_t ezc3d::Matrix::nbCols() const { return _nbCols; }

void ezc3d::Matrix::resize(size_t nbRows, size_t nbCols) {
  _nbRows = nbRows;
  _nbCols = nbCols;
  _data.resize(_nbRows * _nbCols);
}

double ezc3d::Matrix::operator()(size_t row, size_t col) const {
#ifndef USE_MATRIX_FAST_ACCESSOR
  // The data are arrange column majors
  if (nbRows() <= row || nbCols() <= col) {
    throw std::runtime_error(
        "Element ouside of the matrix bounds.\n"
        "Elements ask = " +
        std::to_string(row) + "x" + std::to_string(col) +
        "\nMatrix dimension = " + std::to_string(nbRows()) + "x" +
...
double &ezc3d::Matrix::operator()(size_t row, size_t col) {
#ifndef USE_MATRIX_FAST_ACCESSOR
  // The data are arrange column majors
  if (nbRows() <= row || nbCols() <= col) {
    throw std::runtime_error(
...
ezc3d::Matrix ezc3d::Matrix::operator+(double scalar) {
...
ezc3d::Matrix ezc3d::Matrix::operator+(const ezc3d::Matrix &other) {
  ezc3d::Matrix result(*this);
  return result += other;
}
...
ezc3d::Matrix operator+(double scalar, ezc3d::Matrix mat) {
  return mat + scalar;
}
...
std::ostream &operator<<(std::ostream &out, const ezc3d::Matrix &m) {
  out << "[";
