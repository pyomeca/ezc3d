...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
  ...
  if (nrhs >= 1 && (!mxIsChar(prhs[0]) && !mxIsClass(prhs[0], "string")))
    mexErrMsgTxt(
        "First input argument must be a file path character array or a string "
  ...
  if (nrhs >= 1) {
    ...
    mxArray *charArray = nullptr;

    if (mxIsChar(input)) {
      // Already a character array
      charArray = const_cast<mxArray *>(input);
    } else if (mxIsClass(input, "string")) {
      // Convert string -> char by calling MATLAB's char()
      if (mexCallMATLAB(1, &charArray, 1, const_cast<mxArray **>(&input),
                        "char") != 0) {
        mexErrMsgIdAndTxt("ezc3d:InvalidInput",
                          "Failed to convert string to char.");
      }
    } else {
      mexErrMsgIdAndTxt(
          "ezc3d:InvalidInput",
          "First argument must be a character vector or a string scalar.");
    }

    char *buffer = mxArrayToString(charArray);
    if (buffer == nullptr) {
      if (charArray != input) {
        mxDestroyArray(charArray);
      }
      mexErrMsgIdAndTxt("ezc3d:InvalidInput",
                        "Could not convert input to a C string.");
    }
  ...
  plhs[0] = mxCreateStructArray(2, globalFieldsDims,
                                sizeof(globalFieldsNames) /
                                    sizeof(*globalFieldsNames),
                                globalFieldsNames);
  ...
  try {
    ...
    // Fill the header
    {
      const char *headerFieldsNames[] = {"points", "analogs", "rotations",
                                         "events"};
      ...
      // fill points
      {
        const char *pointsFieldsNames[] = {"size", "frameRate", "firstFrame",
                                           "lastFrame"};
      ...
      // fill analogs
      {
        ...
        fillMatlabField(analogsStruct, 1,
                        static_cast<mxDouble>(c3d->header().nbAnalogByFrame() *
                                              c3d->header().frameRate()));
        fillMatlabField(
            analogsStruct, 2,
            c3d->header().nbAnalogByFrame() * c3d->header().firstFrame() + 1);
      ...
      // fill rotations
      {
        const char *rotationsFieldsNames[] = {"size", "frameRate", "firstFrame",
                                              "lastFrame"};
        ...
        fillMatlabField(rotationsStruct, 1,
                        static_cast<mxDouble>(rotationsInfo.ratio() *
                                              c3d->header().frameRate()));
      ...
      // fill events
      {
        ...
        fillMatlabField(eventsStruct, 0,
                        static_cast<int>(c3d->header().eventsTime().size()));
    ...
    // Fill the parameters
    {
      ...
      for (size_t g = 0; g < nbGroups; ++g) {
        ...
        ezc3d::ParametersNS::GroupNS::Group group(c3d->parameters().group(g));
        ...
        for (size_t p = 0; p < nbParams; ++p) {
          parametersFieldsNames[p + 1] =
              new char[c3d->parameters().group(g).parameter(p).name().length() +
                       1];
          strcpy(parametersFieldsNames[p + 1],
                 c3d->parameters().group(g).parameter(p).name().c_str());
        }
        mwSize parametersFieldsDims[2] = {1, 1};
        mxArray *parametersStruct = mxCreateStructArray(
            2, parametersFieldsDims, static_cast<int>(nbParams + 1),
            const_cast<const char **>(parametersFieldsNames));
        mxSetFieldByNumber(groupsStruct, 0, static_cast<int>(g),
                           parametersStruct);

        // Fill Metadata
        fillMetadata(parametersStruct, 0, group.description(), group.isLocked(),
                     false);
        ...
        for (size_t p = 0; p < nbParams; ++p) {
          ...
          mxArray *valueArray =
              fillMetadata(parametersStruct, p + 1, param.description(),
                           param.isLocked(), true);
          if (param.type() == ezc3d::DATA_TYPE::INT)
            fillMatlabField(valueArray, idxValue, param.valuesAsInt(),
                            param.dimension());
          else if (param.type() == ezc3d::DATA_TYPE::FLOAT)
            fillMatlabField(valueArray, idxValue, param.valuesAsDouble(),
                            param.dimension());
          else if (param.type() == ezc3d::DATA_TYPE::CHAR)
            fillMatlabField(valueArray, idxValue, param.valuesAsString(),
                            param.dimension());
        }
      }
    }
    ...
    // Fill the data
    {
      ...
      // Fill the point data and analogous data
      {
        ...
        mxArray *dataPoints = mxCreateNumericArray(3, dataPointsFieldsDims,
        ...
        mxArray *dataMetaPointsStruct = mxCreateStructArray(
            2, dataMetaPointsFieldsDims, 2, dataMetaPointsFieldsNames);
        ...
        size_t nFramesAnalogs(static_cast<size_t>(
        ...
        mxArray *dataAnalogs =
            mxCreateDoubleMatrix(nFramesAnalogs, nAnalogs, mxREAL);
        double *valAnalogs = mxGetPr(dataAnalogs);

        ezc3d::DataNS::RotationNS::Info rotationsInfo(*c3d);
        size_t nRotations(rotationsInfo.used());
        size_t nFramesRotations(static_cast<mwSize>(c3d->header().nbFrames() *
                                                    rotationsInfo.ratio()));
        size_t nSubFramesRotations(static_cast<mwSize>(rotationsInfo.ratio()));
        size_t nDataRotations[4] = {4, 4, nRotations, nFramesRotations};
        mxArray *dataRotations =
            mxCreateNumericArray(4, nDataRotations, mxDOUBLE_CLASS, mxREAL);
        ...
        for (size_t f = 0; f < nFramesPoints; ++f) {
          ...
          for (size_t p = 0; p < frame.points().nbPoints(); ++p) {
            ...
            if (point.residual() < 0) {
              valPoints[f * nPoints * 3 + 3 * p + 0] = static_cast<double>(NAN);
              valPoints[f * nPoints * 3 + 3 * p + 1] = static_cast<double>(NAN);
              valPoints[f * nPoints * 3 + 3 * p + 2] = static_cast<double>(NAN);
            } else {
            ...
            // Metadata for points
            valMetaResiduals[f * nPoints + p] =
                static_cast<double>(point.residual());
            std::vector<bool> cameraMasks(point.cameraMask());
            for (size_t cam = 0; cam < 7; ++cam) {
              valMetaCameraMasks[f * nPoints * 7 + 7 * p + cam] =
                  cameraMasks[cam];
            }
          }
          ...
          for (size_t sf = 0; sf < frame.analogs().nbSubframes(); ++sf)
            for (size_t c = 0; c < frame.analogs().subframe(sf).nbChannels();
                 ++c)
              valAnalogs[c * nSubFramesAnalogs * nFramesPoints + sf +
                         f * nSubFramesAnalogs] =
                  static_cast<double>(
                      frame.analogs().subframe(sf).channel(c).data());

          // Rotations side
          for (size_t sf = 0; sf < frame.rotations().nbSubframes(); ++sf)
            for (size_t r = 0; r < frame.rotations().subframe(sf).nbRotations();
                 ++r) {
              const ezc3d::DataNS::RotationNS::Rotation &current(
                  frame.rotations().subframe(sf).rotation(r));
              for (size_t i = 0; i < 4; ++i)
                for (size_t j = 0; j < 4; ++j)
                  valRotations[f * nSubFramesRotations * 16 * nRotations +
                               sf * 16 * nRotations + r * 16 + i * 4 + j] =
                      current(j, i);
            }
        }
    ...
    if (nlhs > 1) {
      ...
      const char *forcePlatformNames[] = {
          ...
          "cal_matrix", "corners",     "origin",
          "force",      "moment",      "center_of_pressure",
          "Tz"};
      plhs[1] = mxCreateStructArray(2, globalPlatFormDims,
                                    sizeof(forcePlatformNames) /
                                        sizeof(*forcePlatformNames),
                                    forcePlatformNames);
      ...
      for (size_t i = 0; i < all_pf.forcePlatforms().size(); ++i) {
        ...
        // Units
        mxSetFieldByNumber(plhs[1], i, 0,
                           mxCreateString(pf.forceUnit().c_str()));
        mxSetFieldByNumber(plhs[1], i, 1,
                           mxCreateString(pf.momentUnit().c_str()));
        mxSetFieldByNumber(plhs[1], i, 2,
                           mxCreateString(pf.positionUnit().c_str()));

        // Force platform configuration
        mwSize pfCalMatrixSize[2] = {pf.calMatrix().nbRows(),
                                     pf.calMatrix().nbCols()};
        mxArray *pfCalMatrix =
            mxCreateNumericArray(2, pfCalMatrixSize, mxDOUBLE_CLASS, mxREAL);
        ...
        mxArray *pfCorners =
            mxCreateNumericArray(2, pfCornersSize, mxDOUBLE_CLASS, mxREAL);
        ...
        mxArray *pfOrigin =
            mxCreateNumericArray(2, pfOriginSize, mxDOUBLE_CLASS, mxREAL);
        ...
        // Data
        size_t nFrames(c3d->header().nbFrames() *
                       c3d->header().nbAnalogByFrame());
        mwSize pfDataSize[2] = {3, nFrames};
        mxArray *pfForceMatrix =
            mxCreateNumericArray(2, pfDataSize, mxDOUBLE_CLASS, mxREAL);
        mxArray *pfMomentMatrix =
            mxCreateNumericArray(2, pfDataSize, mxDOUBLE_CLASS, mxREAL);
        mxArray *pfCoPMatrix =
            mxCreateNumericArray(2, pfDataSize, mxDOUBLE_CLASS, mxREAL);
        mxArray *pfTzMatrix =
